...
import { Users, Plus, Loader2, MoreHorizontal, Mail, Clock, X, UserMinus, XCircle } from "lucide-react";
...
export default function TenantAdminUsersPage() {
  ...
  const checkAccess = async () => {
    try {
      ...
      if (user?.role === "SUPER_ADMIN" || user?.role === "super_admin") {
        ...
        fetchRoles(); // Call separately to ensure roles are set
        return;
      }
      ...
      if (tenant.admin_email !== user?.email) {
        toast({
          title: "Access Denied",
          description: "You don't have permission to access this tenant's users",
          variant: "destructive",
        });
        return;
      }
  ...
  const fetchUsers = async () => {
    try {
      ...
      const tenant = tenants.find(t => t.slug === tenantSlug);
      let tenantUsers = usersData.filter(u => u.tenant_id === tenantSlug);
      
      // Fetch all user roles for each user
      const usersWithRoles = await Promise.all(
        tenantUsers.map(async (user) => {
          try {
            const userRoles = await apiClient.request<any[]>(`/user-roles/user/${user.id}`);
      ...
      if (tenant?.admin_email) {
        const tenantAdminExists = usersWithRoles.some(u => u.email === tenant.admin_email);
        if (!tenantAdminExists) {
          try {
            const tenantAdmin = await apiClient.request<AdminUser>(`/users/by-email/${tenant.admin_email}`);
            const userRoles = await apiClient.request<any[]>(`/user-roles/user/${tenantAdmin.id}`).catch(() => []);
            const roleNames = userRoles.map(r => r.role).join(', ');
            ...
            usersWithRoles.unshift(adminWithRoles);
          } catch {
            ...
            usersWithRoles.unshift(tenantAdminEntry);
          }
        }
      }
  ...
  const fetchPendingInvitations = async () => {
    try {
      const invitations = await apiClient.request<PendingInvitation[]>(`/invitations/tenant/${tenantSlug}`);
  ...
  const fetchRoles = async () => {
    try {
      // Use predefined admin roles that match the API enum
      const adminRoles = [
  ...
  const handleChangeRole = async () => {
    ...
    const result = await Swal.fire({
      ...
      text: `This will change ${selectedUser.full_name}'s role to ${roles.find(r => r.value === formData.role)?.label}. They will receive a notification about this change.`,
      icon: 'question',
    ...
    try {
      setSubmitting(true);
      await apiClient.changeUserRole(selectedUser.id, formData.role as any);
  ...
  const handleRemoveRole = async (user: AdminUser, roleToRemove: string) => {
    ...
    const result = await Swal.fire({
      title: 'Remove Role?',
      text: `Remove ${getRoleDisplayName(roleToRemove)} role from ${user.full_name}?`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#dc2626',
      cancelButtonColor: '#6b7280',
    ...
    try {
      ...
      if (userId === 0) {
        const userByEmail = await apiClient.request<AdminUser>(`/users/by-email/${user.email}`);
        userId = userByEmail.id;
      }
      
      const result = await apiClient.request(`/user-roles/remove`, {
        method: "DELETE",
        body: JSON.stringify({ 
          user_id: userId,
  ...
  const handleAddRole = async () => {
    ...
    try {
      ...
      if (userId === 0) {
        const userByEmail = await apiClient.request<AdminUser>(`/users/by-email/${selectedUser.email}`);
        userId = userByEmail.id;
      }
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        <div className="w-full space-y-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600">Manage admin users for your organization</p>
            </div>
            <Button 
                  ...
                  {pendingInvitations.map((invitation) => (
                    <div key={invitation.id} className="flex items-center justify-between p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{invitation.email} • {getRoleDisplayName(invitation.role)}</p>
                  ...
                  {users.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-4 py-12 text-center">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <div className="text-sm font-medium text-gray-900">No admin users found</div>
                          <div className="text-xs text-gray-500">Invite your first admin user to get started</div>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    users.map((user) => (
                      <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap gap-1">
                            {user.role.split(', ').map((role, index) => (
                              <Badge key={index} variant="outline" className="text-xs flex items-center gap-1 pr-1">
                                {getRoleDisplayName(role.trim())}
                                {user.id !== 0 && role.trim() !== "TENANT_ADMIN" && role.trim() !== "GUEST" && (
                                  <button
                                    onClick={() => handleRemoveRole(user, role.trim())}
                        ...
                        <td className="px-4 py-3">
                          <Badge className={`text-xs ${getStatusColor(user.status, user.is_active)}`}>
                        ...
                        <td className="px-4 py-3 text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48 bg-white border border-gray-200 shadow-lg rounded-md">
                              ...
                              {user.id !== 0 && !user.role.includes("TENANT_ADMIN") && (
                                <DropdownMenuItem
        ...
        <Dialog open={showInviteModal} onOpenChange={(open) => !open && closeModals()}>
          <DialogContent className="bg-white border border-gray-200 shadow-lg">
              ...
              <div>
                ...
                <Select value={formData.role} onValueChange={(value) => setFormData({ ...formData, role: value })}>
                  ...
                  <SelectContent className="bg-white border border-gray-200 shadow-lg">
                    {roles.map((role) => (
                      <SelectItem key={role.value} value={role.value} className="hover:bg-gray-50">
                        {role.label}
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            <DialogFooter className="bg-white">
              <Button variant="outline" onClick={closeModals} className="bg-white border-gray-300 text-gray-700 hover:bg-gray-50">
                Cancel
              </Button>
              <Button
        ...
        <Dialog open={showAddRoleModal} onOpenChange={(open) => !open && closeModals()}>
          <DialogContent className="bg-white border border-gray-200 shadow-lg">
            <DialogHeader>
              <DialogTitle className="text-gray-900">Add Role to User</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 bg-white">
              <div>
              ...
              <div>
                <Label htmlFor="add_role" className="text-gray-700">Additional Role</Label>
                <Select value={formData.role} onValueChange={(value) => setFormData({ ...formData, role: value })}>
                  <SelectTrigger className="mt-2 bg-white border-gray-300">
        ...
        <Dialog open={showChangeRoleModal} onOpenChange={(open) => !open && closeModals()}>
          <DialogContent className="bg-white border border-gray-200 shadow-lg">
            <DialogHeader>
            ...
            <div className="space-y-4 bg-white">
              ...
              <div>
                <Label htmlFor="new_role" className="text-gray-700">New Role</Label>
                <Select value={formData.role} onValueChange={(value) => setFormData({ ...formData, role: value })}>
                  <SelectTrigger className="mt-2 bg-white border-gray-300">
            ...
            <DialogFooter className="bg-white">
              ...
              <Button
                ...
                disabled={submitting || !formData.role || formData.role === selectedUser?.role}
                className="bg-red-600 hover:bg-red-700 text-white disabled:opacity-50"
              >
                {submitting ? (
                  <>
